...
async function build(user, backgroundData, levelData) {
  const toAbbrev = (num) => {
  ...
  const backgroundImage = await Canvas.loadImage(backgroundData.background);
  ...
  Canvas.GlobalFonts.registerFromPath("./Functions/Levels/RankCards/Manrope-Bold.ttf", "Manrope Bold");
  const abbreviatedLevel = toAbbrev(parseInt(levelData.level));
  const lb = await expModel
    .find({
      guildID: constantsFile.mainServerID,
    })
    .sort({ level: -1, xp: -1 });
  const rank = lb.findIndex((element) => element.memberID === levelData.memberID) + 1;
  const abbreviatedRank = toAbbrev(parseInt(rank));
